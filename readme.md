# Template Inicial Jest - Método TAJS - Jonatas
...
Na pasta onde você guarda todos os projetos do treinamento, vamos supor que vá chama-lo de `metodo-tajs-jonatas`

```shell
cd metodo-tajs-jonatas
```

e então Restaure os pacotes:

```shell
cd modulo2-setup-jest
npm ci --silent
```
...
Supondo que você está em `metodo-tajs-jonatas/modulo2-setup-jest` e já restaurou os pacotes seguindo os passos anteriores execute:
```shell
code ../
...
```shell
 *  Executing task: bash -c 'PROJECT_DIR=$(dirname /Users/seu-nome/cursos/modulo2-setup-jest/test/template.test.js); cd "$PROJECT_DIR"; npm run test:debug'


> modulo2-setup-jest@0.0.1 test:debug
